...
if __name__ == "__main__":
    ...
    parser.add_argument('--std_target', type=float, help="Standard deviation of the reconstruction Gaussian kernels", default=None)
    ...
    srf = Ngrid // Nmeas
    eps = config["reco_params"]["eps"]
    kernel_std_target =(
        args.std_target if args.std_target is not None
        else config["reco_params"]["kernel_std_target"]
    )
    ...
    if args.l1f is not None and args.l2 is not None:
        ...
        for i, lambda2 in enumerate(args.l2):
            ...
            for j, l1f in enumerate(args.l1f):
                ...
                loss.diff_lipschitz = loss.estimate_diff_lipschitz(method='svd')
