...
if __name__ == "__main__":
    ...
    # plt.show()
    #
    # # Visualization of the sources, separate and the sum
    # locs = np.arange(Ngrid) / Ngrid
    ...
    # plt.subplot(144)
    # plt.stem(locs[img != 0], img[img != 0], markerfmt="^")
    # plt.stem([0, 1 - 1/Ngrid], [0, 0], markerfmt='white')
    # plt.plot(locs, background, c='orange',)
    # plt.title("Composite source")
    # plt.show()
    ...
    if do_non_decoupled: repr_ndcp = np.convolve(x1_ndcp, representation_kernel, mode='same')
    if do_blasso: repr_blasso = np.convolve(x_blasso, representation_kernel, mode='same')
    ...
    if article_plots:
        ...
        # BLASSO solutions: 3 plots, 4 maybe
